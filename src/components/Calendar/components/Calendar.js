...
export default class Calendar extends Component {
 ...
 prepareEventDates(events) {
  ...
  events.forEach(event => {
    ...
    const month = moment(tmpEvent.date).startOf('month').format('YYYY-MM');
    let day = tmpEvent.date.date() - 1;

    parsedDates[month] = parsedDates[month] || {};
    parsedDates[month][day] = parsedDates[month][day] || [];
    parsedDates[month][day].push(tmpEvent);
  });
  ...
  renderMonthView(argMoment, eventsMap) {
    ...
    const events = (eventsMap !== null)
      ? eventsMap[argMoment.startOf('month').format('YYYY-MM')]
      : null;

    console.log('rendering events for month', events)
